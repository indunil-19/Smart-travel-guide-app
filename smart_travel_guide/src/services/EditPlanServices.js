  ...
  if (index > 0) {
    if (index1 > 0) {
      ...
      if (plan[index - 1].length == 0) {
        ...
        start_location =
          plan[index - 1][plan[index - 1].length - 1].geometry.location;
      }
...
export const findPois = async (day, travelPlan, allpois) => {
  ...
  if (num_pois_in_day == 0) {
    ...
    start_location =
      travelPlan[0][day - 1][num_pois_in_day - 1].geometry.location;
  }
...
export const switchPOI = async (
  ...
  for (let i = 0; i < travelPlan[0][index].length; i++) {
    if (index1 != i)
      remaining_time -= travelPlan[1][pivot + i].duration.value + 3600;
  }
  remaining_time -= 3600;
  // console.log(remaining_time)
