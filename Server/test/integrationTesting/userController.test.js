...
describe("user functionalities integration test", ()=>{
    ...
    describe('change password functionality', () => {
        ...
        it("shoud return successs message if no error", async()=>{
            await UserController.changePasssword(req,res)
            expect(res.json).toHaveBeenCalledWith({message:"password update successfull"})
        })

        it("shoud return error if all fields are not filled", async()=>{
            req.body.newPassword=""
            await UserController.changePasssword(req,res)
            expect(res.json).toHaveBeenCalledWith({error:"please add all the fields"})
        })
        it("shoud return error if system fails", async()=>{
            req.session.email={}
            await UserController.changePasssword(req,res)
