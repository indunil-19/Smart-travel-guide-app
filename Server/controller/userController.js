...
class UserController {
  ...
  static async updateTravelPlan(req, res) {
    TravelPlan.findOneAndUpdate(
      { _id: req.body.planId, ownedBy: req.session.user._id },
      {
        travelPlan: req.body.travelPlan,
      }
    )
  ...
  static async getReviews(req, res) {
    TravelPlan.findOne({ _id: req.body.planId, ownedBy: req.session.user._id })
      .then((data) => {
        return res.json({ data });
      })
      .catch((e) => {
  ...
  static async changePasssword(req, res) {
    ...
    return User.findOne({ email: req.session.user.email })
      .then((savedUser) => {
        return bcrypt
          .compare(req.body.prevoiusPassowrd, savedUser.password)
          .then((doMatch) => {
            if (doMatch) {
